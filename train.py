...
def test(model, test_loader, criterion, device):
    ...
    for class_idx in range(num_classes):
        logger.info(f"Class {class_idx + 1}: Precision={class_precision [class_idx]:.4f}, Recall={class_recall[class_idx]:.4f}, F1={class_recall[class_idx]:.4f}")
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for inputs, labels in validation_loader:
                ...
                #Forward pass to obtain predicted
...
if __name__=='__main__':
    ...
    parser.add_argument('--data_dir', type=str, default=os.environ['SM_CHANNEL_TRAINING'], help="Training data path in 53")
    parser.add_argument('-model_dir', type=str, default=os.environ['SM_MODEL_DIR'], help="Model output path in 53")
    parser.add_argument('--output_dir', type=str, default=os.environ['SM_OUTPUT_DATA_DIR'], help="Output path in 53")
